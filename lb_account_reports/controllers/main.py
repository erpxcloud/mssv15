    ...
    def get_report(self, model, options, output_format, token, financial_id=None, **kw):
        ...
        try:
            if output_format == 'xlsx':
                ...
                response.stream.write(report_obj.with_context({'selected_multi_currency': options.get('selected_multi_currency')}).get_xlsx(options))
            if output_format == 'pdf':
                response = request.make_response(
                    report_obj.with_context({'selected_multi_currency': options.get('selected_multi_currency')}).get_pdf(options),
                    headers=[
            ...
            if output_format == 'xml':
                content = report_obj.with_context({'selected_multi_currency': options.get('selected_multi_currency')}).get_xml(options)
                response = request.make_response(
                    content,
                    headers=[
            ...
            if output_format == 'txt':
                content = report_obj.with_context({'selected_multi_currency': options.get('selected_multi_currency')}).get_txt(options)
                response = request.make_response(
                    content,
                    headers=[
            ...
            if output_format == 'csv':
                content = report_obj.with_context({'selected_multi_currency': options.get('selected_multi_currency')}).get_csv(options)
                response = request.make_response(
                    content,
                    headers=[
            ...
            if output_format == 'zip':
                content = report_obj.with_context({'selected_multi_currency': options.get('selected_multi_currency')}).get_zip(options)
                response = request.make_response(
                    content,
                    headers=[
