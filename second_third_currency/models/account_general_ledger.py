...
class GeneralLedgerReport(models.AbstractModel):
    ...
    def _get_options(self, previous_options=None):
        res = super(GeneralLedgerReport, self)._get_options(previous_options)
    ...
    def _get_templates(self):
        templates = super(GeneralLedgerReport, self)._get_templates()
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        cpompany = self.env['res.company'].search([('id', '=', self.env.company.id)])
        second = self.env['res.currency'].search([('id', '=', cpompany.second_currency.id)])
        third = self.env['res.currency'].search([('id', '=', cpompany.third_currency.id)])
        ...
        for account, periods_results in accounts_results:
            ...
            if self._context.get('curr') == second.id:
                ...
                debit = account_sum.get('second_debit_id', 0.0) + account_un_earn.get('second_debit_id', 0.0)
                credit = account_sum.get('second_credit_id', 0.0) + account_un_earn.get('second_credit_id', 0.0)
                balance = account_sum.get('second_balance', 0.0) + account_un_earn.get('second_balance', 0.0)
            elif self._context.get('curr') == third.id:
                amount_currency = account_sum.get('amount_currency', 0.0) + account_un_earn.get('amount_currency', 0.0)
                debit = account_sum.get('third_debit_id', 0.0) + account_un_earn.get('third_debit_id', 0.0)
                credit = account_sum.get('third_credit_id', 0.0) + account_un_earn.get('third_credit_id', 0.0)
                balance = account_sum.get('third_balance', 0.0) + account_un_earn.get('third_balance', 0.0)
            else:
                amount_currency = account_sum.get('amount_currency', 0.0) + account_un_earn.get('amount_currency', 0.0)
            ...
            if has_lines and (unfold_all or is_unfolded):
                ...
                if self._context.get('curr') == second.id:
                    cumulated_balance = account_init_bal.get('second_balance', 0.0) + account_un_earn.get('second_balance', 0.0)
                    ...
                    lines.append(self._get_initial_balance_line(
                        ...
                        account_init_bal.get('second_debit_id', 0.0) + account_un_earn.get('second_debit_id', 0.0),
                        account_init_bal.get('second_credit_id', 0.0) + account_un_earn.get('second_credit_id', 0.0),
                        cumulated_balance,
                    ))
                elif self._context.get('curr') == third.id:
                    cumulated_balance = account_init_bal.get('second_balance', 0.0) + account_un_earn.get('second_balance', 0.0)
                    ...
                    lines.append(self._get_initial_balance_line(
                        options, account,
                        account_init_bal.get('amount_currency', 0.0) + account_un_earn.get('amount_currency', 0.0),
                        account_init_bal.get('third_debit_id', 0.0) + account_un_earn.get('third_debit_id', 0.0),
                        account_init_bal.get('third_credit_id', 0.0) + account_un_earn.get('third_credit_id', 0.0),
                        cumulated_balance,
                    ))
                ...
                for aml in amls:
                    ...
                    lines.append(self._get_aml_line(options, account, aml, company_currency.round(cumulated_balance),currency))
                ...
                if self.env.company.totals_below_sections:
                    ...
                    if self._context.get('curr') == second.id:
                        lines.append(self._get_account_total_line(
                            ...
                            account_sum.get('second_debit_id', 0.0),
                            account_sum.get('second_credit_id', 0.0),
                            account_sum.get('second_balance', 0.0),
                        ))
                    elif self._context.get('curr') == third.id:
                        lines.append(self._get_account_total_line(
                            options, account,
                            account_sum.get('amount_currency', 0.0),
                            account_sum.get('third_debit_id', 0.0),
                            account_sum.get('third_credit_id', 0.0),
                            account_sum.get('third_balance', 0.0),
                        ))
    ...
    def _load_more_lines(self, options, line_id, offset, load_more_remaining, balance_progress):
        ...
        for aml in self._cr.dictfetchall():
            ...
            if load_more_counter == 0:
                break
            if self._context.get('curr') == second.id:
                balance_progress += aml['second_balance']
            elif self._context.get('curr') == third.id:
                balance_progress += aml['third_balance']
            else:
                balance_progress += aml['balance']

            # account.move.line record line.
            lines.append(self._get_aml_line(options, expanded_account, aml, balance_progress,currency))
    ...
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(GeneralLedgerReport, self)._get_options_domain(options)
            ...
            if self._context.get('curr') == second.id:
                queries.append('''
                    SELECT
                        ...
                        SUM(ROUND(account_move_line.second_debit_id , currency_table.precision))   AS second_debit_id,
                        SUM(ROUND(account_move_line.second_credit_id , currency_table.precision))  AS second_credit_id,
                        SUM(ROUND(account_move_line.second_balance, currency_table.precision)) AS second_balance
                    FROM %s
                ...
                queries.append('''
                    SELECT
                        ...
                        %s                                                      AS period_number,
                        COALESCE(SUM(account_move_line.amount_currency), 0.0)   AS amount_currency,
                        SUM(ROUND(account_move_line.third_debit_id , currency_table.precision))   AS third_debit_id,
                        SUM(ROUND(account_move_line.third_credit_id , currency_table.precision))  AS third_credit_id,
                        SUM(ROUND(account_move_line.third_balance, currency_table.precision)) AS third_balance
                    FROM %s
                    LEFT JOIN %s ON currency_table.company_id = account_move_line.company_id
                    WHERE %s
                    GROUP BY account_move_line.account_id
                ...
                if self._context.get('curr') == second.id:
                    ...
                    queries += ['''
                        SELECT
                            ...
                            0.0                                     AS second_debit_id,
                            0.0                                     AS second_credit_id,
                            SUM(ROUND(account_move_line.second_balance, currency_table.precision)) AS second_balance
                    ...
                    queries += ['''
                        SELECT
                            ...
                            %s                                      AS period_number,
                            0.0                                     AS amount_currency,
                            0.0                                     AS third_debit_id,
                            0.0                                     AS third_credit_id,
                            SUM(ROUND(account_move_line.second_balance, currency_table.precision)) AS second_balance
                        FROM account_move_line_account_tax_rel tax_rel, %s
                        LEFT JOIN %s ON currency_table.company_id = account_move_line.company_id
                        WHERE account_move_line.id = tax_rel.account_move_line_id AND %s
        ...
        if self._context.get('curr') == second.id:
            query = f'''
                SELECT
                    ...
                    ROUND(account_move_line.second_debit_id , currency_table.precision)   AS second_debit_id,
                    ROUND(account_move_line.second_credit_id, currency_table.precision)  AS second_credit_id,
                    ROUND(account_move_line.second_balance, currency_table.precision) AS second_balance,
            ...
            query = f'''
                SELECT
                    ...
                    account_move_line.currency_id,
                    account_move_line.amount_currency,
                    ROUND(account_move_line.third_debit_id , currency_table.precision)   AS third_debit_id,
                    ROUND(account_move_line.third_credit_id, currency_table.precision)  AS third_credit_id,
                    ROUND(account_move_line.third_balance, currency_table.precision) AS third_balance,
                    account_move_line__move_id.name         AS move_name,
                    company.currency_id                     AS company_currency_id,
                    partner.name                            AS partner_name,
                    account_move_line__move_id.move_type    AS move_type,
        ...
        for res in self._cr.dictfetchall():
            ...
            if self._context.get('curr') == second.id:
                ...
                if key == 'sum':
                    ...
                    groupby_taxes[res['groupby']][key] = res['second_balance']
                ...
                if key == 'sum':
                    ...
                    groupby_taxes.setdefault(res['groupby'], {})
                    groupby_taxes[res['groupby']][key] = res['third_balance']
                elif key == 'tax_amount' and len(options_list) == 1:
                    groupby_taxes.setdefault(res['groupby'], {})
                    groupby_taxes[res['groupby']][key] = res['third_balance']
            else:
    ...
    def _get_account_title_line(self, options, account, amount_currency, debit, credit, balance, has_lines):
        ...
        columns = [
            ...
            {'name': self.format_value(balance,currency = self.env['res.currency'].browse(self._context.get('curr'))), 'class': 'number'},
        ]
        if self.user_has_groups('base.group_multi_currency'):
            has_foreign_currency = account.currency_id and account.currency_id != account.company_id.currency_id or False
            columns.insert(0, {'name': has_foreign_currency and self.format_value(amount_currency,currency = self.env['res.currency'].browse(self._context.get('curr')), blank_if_zero=True) or '', 'class': 'number'})
        return {
    ...
    @api.model
    def _get_aml_line(self, options, account, aml, cumulated_balance,currency):
        cpompany = self.env['res.company'].search([('id', '=', self.env.company.id)])
        second = self.env['res.currency'].search([('id', '=', cpompany.second_currency.id)])
        third = self.env['res.currency'].search([('id', '=', cpompany.third_currency.id)])
        if aml['payment_id']:
        ...
        if self._context.get('curr') == second.id:
            columns = [
                ...
                {'name': self.format_value(aml['second_debit_id'],currency = self.env['res.currency'].browse(self._context.get('curr')), blank_if_zero=True), 'class': 'number'},
                {'name': self.format_value(aml['second_credit_id'],currency = self.env['res.currency'].browse(self._context.get('curr')), blank_if_zero=True), 'class': 'number'},
                {'name': self.format_value(cumulated_balance,currency = self.env['res.currency'].browse(self._context.get('curr'))), 'class': 'number'},
            ]
        elif self._context.get('curr') == third.id:
            columns = [
                {'name': format_date(self.env, aml['date']), 'class': 'date'},
                {'name': self._format_aml_name(aml['name'], aml['ref']), 'class': 'o_account_report_line_ellipsis'},
                {'name': aml['partner_name'], 'class': 'o_account_report_line_ellipsis'},
                {'name': self.format_value(aml['third_debit_id'],currency = self.env['res.currency'].browse(self._context.get('curr')), blank_if_zero=True), 'class': 'number'},
                {'name': self.format_value(aml['third_credit_id'],currency = self.env['res.currency'].browse(self._context.get('curr')), blank_if_zero=True), 'class': 'number'},
                {'name': self.format_value(cumulated_balance,currency = self.env['res.currency'].browse(self._context.get('curr'))), 'class': 'number'},
            ]
        else:
            columns = [
                ...
                {'name': self._format_aml_name(aml['name'], aml['ref']), 'class': 'o_account_report_line_ellipsis'},
                {'name': aml['partner_name'], 'class': 'o_account_report_line_ellipsis'},
                {'name': self.format_value(aml['debit'],currency = self.env['res.currency'].browse(self._context.get('curr')), blank_if_zero=True), 'class': 'number'},
                {'name': self.format_value(aml['credit'],currency = self.env['res.currency'].browse(self._context.get('curr')), blank_if_zero=True), 'class': 'number'},
                {'name': self.format_value(cumulated_balance,currency = self.env['res.currency'].browse(self._context.get('curr'))), 'class': 'number'},
            ]
        if self.user_has_groups('base.group_multi_currency'):
    ...
    def _get_account_total_line(self, options, account, amount_currency, debit, credit, balance):
        ...
        if self.user_has_groups('base.group_multi_currency'):
            has_foreign_currency = account.currency_id and account.currency_id != account.company_id.currency_id or False
            columns.append({'name': has_foreign_currency and self.format_value(amount_currency,currency = self.env['res.currency'].browse(self._context.get('curr')), blank_if_zero=True) or '', 'class': 'number'})
    ...
    def get_xlsx(self, options, response=None):
        for opt in options['currenciess']:
            if opt['selected'] and self.env['res.currency'].browse(opt['id']) != self.env.user.company_id.currency_id:
                return super(GeneralLedgerReport, self.with_context(curr = opt['id'])).get_xlsx(options,response)
        return super(GeneralLedgerReport, self).get_xlsx(options,response)
