...
class ReportAccountAgedPartner(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        columns = [
            ...
            {'name': _("121 - 240"), 'class': 'number sortable', 'style': 'white-space:nowrap;'},
            {'name': _("241 - 360"), 'class': 'number sortable', 'style': 'white-space:nowrap;'},
    ...
    def _get_lines(self, options, line_id=None):
        ...
        results2, total2, amls2 = self.env['report.account.report_agedpartnerbalance'].with_context(
            **context)._get_partner_move_lines(account_types, self._context['date_to'], 'posted', 120)

        for values, values2 in zip(results, results2):
            #             _logger.info(
            #                  f'\n\n\n\n\n,{values2},\n-----------------------------\n\n\n\n.')
            #             _logger.info(
            #                  f'\n\n\n\n\n,{values},\n******************************\n\n\n\n.')
            vals = {
                ...
                'columns': [{'name': ''}] * 4 + [{'name': self.format_value(sign * v), 'no_format': sign * v}
                                                 for v in [values['direction'], values['4'],
                                                           values['3'], values['2'],
                                                           values['1'], values2['3'], values2['2'], values2['0'],
                                                           values['total']]],
