...
class ReportJournal(models.AbstractModel):
    ...
    def lines(self, target_move, journal_ids, sort_selection, data):
        ...
        query = """SELECT "account_move_line".id FROM param_1, account_move am, account_account acc
                WHERE "account_move_line".account_id = acc.id AND "account_move_line".move_id=am.id AND am.state IN %s AND "account_move_line".journal_id IN %s AND param_2
                ORDER BY """
        ...
        query = query.replace("param_1", query_get_clause[0])
        query = query.replace("param_2", query_get_clause[1])
    ...
    def _sum_debit(self, data, journal_id):
        ...
        query = """SELECT SUM(debit) FROM param_1, account_move am
            WHERE "account_move_line".move_id=am.id AND am.state IN %s AND "account_move_line".journal_id IN %s AND param_2 """
    ...
    def _sum_credit(self, data, journal_id):
        ...
        query_get_clause = self._get_query_get_clause(data)
        params = [tuple(move_state), tuple(journal_id.ids)] + query_get_clause[2]
        query = """SELECT SUM(credit) FROM param_1, account_move am
            WHERE "account_move_line".move_id=am.id AND am.state IN %s AND "account_move_line".journal_id IN %s AND param_2 """
    ...
    def _get_taxes(self, data, journal_id):
        ...
        query = """
            ...
            FROM account_move_line_account_tax_rel rel, param_1
        ...
        for tax in self.env["account.tax"].browse(ids):
            query = """SELECT sum(debit - credit) FROM param_1, account_move am "
                WHERE "account_move_line".move_id=am.id AND am.state IN %s AND "account_move_line".journal_id IN %s AND
                param_2 AND tax_line_id = %s """

            query = query.replace("param_1", query_get_clause[0])
            query = query.replace("param_2", query_get_clause[1])

            self.env.cr.execute(
                query, tuple(params + [tax.id]),
            )
            res[tax] = {
