...
class HrContract(models.Model):
    ...
    thirteen_month = fields.Monetary(string='13th Month',
    ...
    travel_amount = fields.Monetary("Travel Amount", help="Daily amount the employee receives for each Travel day")
        
    currency_id = fields.Many2one(string="Currency", related='company_id.payroll_currency_id', readonly=True)
    
    @api.onchange('wage')
    def _onchange_wage_value(self):
    ...
    def _get_mobile_amount(self, has_mobile, international_communication):
        if has_mobile and international_communication:
            return self.mobile + self.mobile_plus
        elif has_mobile:
            return self.mobile
        else:
            return 0.0
...
class HrEmployee(models.Model):
    ...
    NSSF = fields.Char(string='NSSF', groups="hr.group_hr_user")
    MOF = fields.Char(string='MOF', groups="hr.group_hr_user")
    subjected_to_nssf = fields.Boolean(string='Subjected to NSSF',default=True, groups="hr.group_hr_user")
